...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <div class="collapse navbar-collapse" id="navbarNavDropdown">
        ...
        <%
            ...
            request.getSession().setAttribute("url","init.search");
        %>
        <li class="nav-item">
...
<main>
    ...
    <div class="col-12  bg-light text-dark p-4 mb-4">
        <form action="searching.search" name="Search" method="post" onsubmit="">
            <input class="inputBox" type="text" name="searcher" id="searcher" value="${param.searcher}"><br>
            <%
                if("byDescription".equals(request.getParameter("filter"))){
            %>
            <input type="radio" name="filter" value="byTitle">filter by title<br>
            <input type="radio" name="filter" value="byDescription" checked>filter by content<br>
            <%
                }else{
            %>
            <input type="radio" name="filter" value="byTitle" checked>filter by title<br>
            <input type="radio" name="filter" value="byDescription">filter by content<br>
            <%
                }
                if("byTime".equals(request.getParameter("order"))){
            %>
            <input type="radio" name="order" value="byPop">filter by popularity<br>
            <input type="radio" name="order" value="byTime" checked>filter by time<br>
            <%
            }else{
            %>
            <input type="radio" name="order" value="byPop" checked>filter by popularity<br>
            <input type="radio" name="order" value="byTime">filter by time<br>
            <%
                }
            %>
    ...
    <div id="resultBox">
        ...
        %>
        <p>Nothing found.Please try another key word.</p>
        <%
                ...
                <div class="col-8">
                    <div class="card-body">
                        ...
                        <p class="card-text shortContent">
                            ...
                            <%
                            if(thisPageResult.get(i).getDescription()!=null){
                            %>
    ...
    <nav class="m-auto">
        <ul class="pagination" id="pageBox">
            ...
            <%
                if(presentPage!=1){
            %>
            <li class="page-item"><a class="page-link" href="pageShow.search?searcher=<%=request.getParameter("searcher")%>&filter=<%=request.getParameter("filter")%>&order=<%=request.getParameter("order")%>&pageAll=<%=request.getParameter("pageAll")%>&page=<%=(presentPage-1)%>">Previous</a></li>
            <%
                }
            ...
            %>
            <a class="page-link" href="pageShow.search?searcher=<%=request.getParameter("searcher")%>&filter=<%=request.getParameter("filter")%>&order=<%=request.getParameter("order")%>&pageAll=<%=request.getParameter("pageAll")%>&page=<%=i%>">
                <%=i%>
            </a>
        </li>
            <%
            ...
            %>
            <li class="page-item"><a class="page-link" href="pageShow.search?searcher=<%=request.getParameter("searcher")%>&filter=<%=request.getParameter("filter")%>&order=<%=request.getParameter("order")%>&pageAll=<%=request.getParameter("pageAll")%>&page=<%=(presentPage+1)%>">Next</a></li>
            <%
                }
                }
            %>
