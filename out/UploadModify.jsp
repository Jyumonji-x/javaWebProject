...
<%@ page import="com.project.dao.ImageDao" %>
...
<main>
    ...
    <%
        TravelImage travelImage = new ImageDaoImpl().getImageById(Integer.parseInt(request.getParameter("imageId")));
    %>
    <form class="col-12  bg-light text-dark p-4 mb-4" enctype="multipart/form-data" action="modify.upload?imageId=<%=request.getParameter("imageId")%>" method="post" id="form" onsubmit="return UploadConfirm()">
        <div class="col-12  bg-light text-dark p-4 mb-4" id="photoUploadFrame">
            <img class="photoUpload img-fluid" id="imageUpload" src="img/images/<%=travelImage.getPath()%>">
        </div>
        <input class="inputBox" placeholder="Title" type="text" name="title" id="titleBox" value="<%=travelImage.getTitle()%>" required><br>
        <textarea class="inputBox" rows="5" cols="60" placeholder="Description" name="description" id="descriptionBox" required><%=travelImage.getDescription()%></textarea><br>
        <input class="select" name="content" type="text" id="contents" placeholder="Content" value="<%=travelImage.getContent()%>" required>
        <select class="select" name="country" id="countries">
            ...
            <%
                }System.out.println(travelImage.getCountry_RegionCodeISO());
        ...
        <script>
            ...
            for(let i=0;i<countryList.length;i++){
                if(countryList[i].value==="<%=travelImage.getCountry_RegionCodeISO()%>")
    ...
    $(document).ready(function () {
        $.get("citySelect.upload",{iso:$("#countries option:selected").val()}, function (data) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if(data[i].geoNameID=== <%=travelImage.getCityCode()%>)
                    line.selected = true;
